    ...
    ctx.withStackFrame(name, isNative = true) {
      try body
        ...
        case jsEx: quickjs.runtime.JSException =>
          jsEx.getValue match
            case JSValue.Object(obj) => if ctx.isErrorObject(obj) then forceStack(obj)
            case _ => ()
          throw jsEx
  ...
  def call(
    ...
    if function.isGenerator then
      val funcValue = JSValue.Function(
        name = function.name, bytecode = function.bytecode, constants = function.constants,
        stackSize = function.stackSize, closure = closure.clone(),
        paramNames = function.paramNames, localVarNames = function.localVarNames,
        parentLocalVarNames = Array.empty, argumentsIndex = function.argumentsIndex,
        isConstructor = function.isConstructor, isGenerator = function.isGenerator,
        isAsync = function.isAsync, funcObj = quickjs.objmodel.JSObject(),
        spanMap = function.spanMap, isStrict = function.isStrict)
      val varsArray = new Array[JSValue](256)
      for i <- 0 until 256 do varsArray(i) = JSValue.Undefined
      val gen = JSValue.Generator(
        func = funcValue, state = JSValue.GeneratorState.SuspendedStart,
        suspendedPc = 0, stack = new Array[JSValue](function.stackSize), stackTop = 0,
        args = args.clone(), vars = varsArray, thisArg = thisArg,
        closure = closure.clone(), pendingValue = JSValue.Undefined)
    ...
    if function.isAsync then
      ...
      val nonAsyncFunction = new BytecodeFunction(
        name = function.name, bytecode = function.bytecode, constants = function.constants,
        stackSize = function.stackSize, freeVars = function.freeVars,
        paramNames = function.paramNames, localVarNames = function.localVarNames,
        argumentsIndex = function.argumentsIndex, isConstructor = function.isConstructor,
        isGenerator = function.isGenerator, isAsync = false,
        length = function.length, spanMap = function.spanMap, isStrict = function.isStrict)
      try
        ...
        case e: quickjs.runtime.JSException =>
          promise.state = JSValue.PromiseState.Rejected; promise.result = e.getValue
    ...
    ctx.withStackFrame(frameName, isNative = false, spanMap = function.spanMap):
      ...
      if trace.isEnabled then
        trace.recordCall(CallTrace(frameName, args.toVector.map(arg => TraceValue.from(arg))))

      val loop = new BytecodeLoop(
        interpreter = this,
        frame = new Frame(
          stack = stack, stackTop = stackTop, pc = 0, bytecode = function.bytecode,
          locals = locals, localsCount = localsCount, thisValue = thisValue,
          closure = closure, withStack = withStack,
...
object Interpreter:
  ...
  private[interpreter] def readInt64(buf: Array[Byte], pc: Int): Long =
    ...
    ((buf(pc + 2).toLong & 0xFF) << 40) | ((buf(pc + 3).toLong & 0xFF) << 32) |
    ((buf(pc + 4).toLong & 0xFF) << 24) | ((buf(pc + 5).toLong & 0xFF) << 16) |
  ...
  private[interpreter] def looseEqual(a: JSValue, b: JSValue): Boolean = (a, b) match
    ...
    case (JSValue.BigInt(x), _) if b.isNumber =>
      val nb = b.toNumber; !nb.isNaN && new java.math.BigDecimal(x).compareTo(new java.math.BigDecimal(nb)) == 0
    case (_, JSValue.BigInt(y)) if a.isNumber =>
      val na = a.toNumber; !na.isNaN && new java.math.BigDecimal(na).compareTo(new java.math.BigDecimal(y)) == 0
    case (JSValue.BigInt(_), _) | (_, JSValue.BigInt(_)) => false
    case (JSValue.Undefined, JSValue.Null) | (JSValue.Null, JSValue.Undefined) => true
    case (_: JSValue.JSStr, _: JSValue.JSStr) => a.toString == b.toString
    case (_: JSValue.Bool, _) | (_, _: JSValue.Bool) => a.toNumber == b.toNumber
    case (_: JSValue.JSStr, _: JSValue.Int32) | (_: JSValue.JSStr, _: JSValue.Float64) => a.toNumber == b.toNumber
    case (_: JSValue.Int32, _: JSValue.JSStr) | (_: JSValue.Float64, _: JSValue.JSStr) => a.toNumber == b.toNumber
  ...
  private[interpreter] def arrayToStringNative(arrVal: JSValue.JSArrayVal): JSValue =
    JSValue.Native(quickjs.value.NativeFunction(name = "toString",
      impl = (args, _) => args.headOption match
        case Some(arr: JSValue.JSArrayVal) =>
          val arrObj = arr.value; val sb = new StringBuilder(); var i = 0
          while i < arrObj.getLength do { if i > 0 then sb.append(","); sb.append(arrObj.get(i).toString); i += 1 }
  ...
  private[interpreter] def primitiveToStringNative(name: String, value: JSValue): JSValue =
    JSValue.Native(quickjs.value.NativeFunction(name = name,
      impl = (args, _) => args.headOption match
