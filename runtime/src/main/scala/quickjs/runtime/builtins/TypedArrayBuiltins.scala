...
object TypedArrayBuiltins {
  ...
  private def toNativeGetter(name: String)(f: JSValue => JSContext ?=> JSValue): JSValue =
    JSValue.Native(NativeFunction(name = name, impl = (args, ctx) => { given JSContext = ctx; f(args(0))(using ctx) }))
    ...
    val abCtor = quickjs.value.NativeConstructor(
      ...
      constructImpl = (args, ctx) => {
        ...
        val length = if (args.nonEmpty) {
          ...
          if (d < 0 || d.isNaN || d.isInfinite) ctx.throwRangeError("Invalid array buffer length")
          d.toInt
        } else 0
  ...
  private def initializeTypedArray(ctx: JSContext, typ: TypedArrayType): Unit = {
    given JSContext = ctx
    val taProto = JSObject(prototype = ctx.objectPrototype)
    ...
    taProto.initAccessorProperty("buffer",
      getter = Some(toNativeGetter("get buffer") { thisVal => thisVal match {
        case JSValue.Object(obj) => getView(obj) match {
          case Some((view, _)) => findOrCreateBufferObject(view.buffer)
          case None => ctx.throwTypeError(s"Expected ${typ.className}")
        }
        case _ => ctx.throwTypeError(s"Expected ${typ.className}")
      }}), setter = None, enumerable = false, configurable = true)

    taProto.initAccessorProperty("byteLength",
      getter = Some(toNativeGetter("get byteLength") { thisVal => thisVal match {
        case JSValue.Object(obj) => getView(obj) match {
          case Some((view, _)) => JSValue.fromInt(view.byteLength)
          case None => ctx.throwTypeError(s"Expected ${typ.className}")
        }
        case _ => ctx.throwTypeError(s"Expected ${typ.className}")
      }}), setter = None, enumerable = false, configurable = true)

    taProto.initAccessorProperty("byteOffset",
      getter = Some(toNativeGetter("get byteOffset") { thisVal => thisVal match {
        case JSValue.Object(obj) => getView(obj) match {
          case Some((view, _)) => JSValue.fromInt(view.byteOffset)
          case None => ctx.throwTypeError(s"Expected ${typ.className}")
        }
        case _ => ctx.throwTypeError(s"Expected ${typ.className}")
      }}), setter = None, enumerable = false, configurable = true)

    taProto.initAccessorProperty("length",
      getter = Some(toNativeGetter("get length") { thisVal => thisVal match {
        case JSValue.Object(obj) => getView(obj) match {
          case Some((view, _)) => JSValue.fromInt(view.length)
          case None => ctx.throwTypeError(s"Expected ${typ.className}")
        }
        case _ => ctx.throwTypeError(s"Expected ${typ.className}")
      }}), setter = None, enumerable = false, configurable = true)
    ...
    // set method
    taProto.initProperty("set", toNativeFn("set", 2) { args => args(0) match {
      case JSValue.Object(obj) => getView(obj) match {
        case Some((view, _)) =>
    ...
    // subarray method
    taProto.initProperty("subarray", toNativeFn("subarray", 2) { args => args(0) match {
      case JSValue.Object(obj) => getView(obj) match {
        case Some((view, _)) =>
          ...
          createTypedArrayView(typ, view.buffer, view.byteOffset + newBegin * typ.bytesPerElement, newLen)
        case None => ctx.throwTypeError(s"Expected ${typ.className}")
      }
      case _ => ctx.throwTypeError(s"Expected ${typ.className}")
    }}, enumerable = false, writable = true, configurable = true)
    ...
    // slice method
    taProto.initProperty("slice", toNativeFn("slice", 2) { args => args(0) match {
      case JSValue.Object(obj) => getView(obj) match {
        case Some((view, _)) =>
          val begin = if (args.length > 1) { val b = args(1).toNumber.toInt; if (b < 0) math.max(0, view.length + b) else b } else 0
          val end = if (args.length > 2) { val e = args(2).toNumber.toInt; if (e < 0) math.max(0, view.length + e) else e } else view.length
          val newBegin = math.max(0, math.min(begin, view.length))
          val newLen = math.max(0, math.min(end, view.length) - newBegin)
          val byteLen = newLen * typ.bytesPerElement
          val newStorage = ArrayBufferStorage(byteLen)
          System.arraycopy(view.buffer.data, view.byteOffset + newBegin * typ.bytesPerElement, newStorage.data, 0, byteLen)
          createTypedArrayFromStorage(typ, newStorage, 0, byteLen)
        case None => ctx.throwTypeError(s"Expected ${typ.className}")
      }
      case _ => ctx.throwTypeError(s"Expected ${typ.className}")
    }}, enumerable = false, writable = true, configurable = true)
    ...
    getWellKnownSymbol("toStringTag") match {
      case JSValue.Symbol(id) =>
        taProto.initSymbolProperty(id, JSValue.fromString(typ.className), enumerable = false, writable = false, configurable = true)
      case _ => ()
    }
  ...
  private def initializeDataView(ctx: JSContext): Unit = {
    ...
    getWellKnownSymbol("toStringTag") match {
      case JSValue.Symbol(id) =>
        dvProto.initSymbolProperty(id, JSValue.fromString("DataView"), enumerable = false, writable = false, configurable = true)
      case _ => ()
    }
