...
object BigIntBuiltins:
  ...
  private def toPrimitiveNumber(obj: quickjs.objmodel.JSObject)(using ctx: JSContext): JSValue =
    def isPrimitive(v: JSValue): Boolean = v match
      case _: JSValue.Undefined.type | _: JSValue.Null.type | _: JSValue.Bool | _: JSValue.Int32 | _: JSValue.Float64 | _: JSValue.JSStr | _: JSValue.BigInt | _: JSValue.Symbol => true
      case _ => false
          ...
          case JSValue.Native(nf: quickjs.value.NativeFunction) =>
            ...
            if isPrimitive(result) then result
              ...
              obj.get("toString")(using ctx) match
                case JSValue.Native(tsf: quickjs.value.NativeFunction) => tsf.call(Array(JSValue.Object(obj)))
                case _ => ctx.throwTypeError("Cannot convert object to primitive value")
          case _ =>
            obj.get("toString")(using ctx) match
              case JSValue.Native(tsf: quickjs.value.NativeFunction) => tsf.call(Array(JSValue.Object(obj)))
              case _ => ctx.throwTypeError("Cannot convert object to primitive value")
  ...
  private def toBigInt(value: JSValue)(using ctx: JSContext): java.math.BigInteger = value match
    case JSValue.BigInt(b) => b
    case JSValue.Bool(b) => if b then java.math.BigInteger.ONE else java.math.BigInteger.ZERO
    case JSValue.JSStr(s) =>
      ...
      if trimmed.isEmpty then java.math.BigInteger.ZERO
        ...
        try
          ...
          case _: NumberFormatException => ctx.throwSyntaxError(s"Cannot convert $s to a BigInt")
    case JSValue.Int32(i) =>
      java.math.BigInteger.valueOf(i.toLong)
    case JSValue.Float64(d) =>
      if d.isNaN || d.isInfinite then ctx.throwRangeError("The number cannot be converted to a BigInt because it is not an integer")
      if d != d.floor then ctx.throwRangeError("The number cannot be converted to a BigInt because it is not an integer")
  ...
  private def toBigIntStrict(value: JSValue)(using ctx: JSContext): java.math.BigInteger = value match
    case JSValue.BigInt(b) => b
    case JSValue.Bool(b) => if b then java.math.BigInteger.ONE else java.math.BigInteger.ZERO
    case JSValue.JSStr(s) =>
      val trimmed = s.trim()
  ...
  /** ToIndex abstract operation: if index < 0 or is Infinity/NaN, throw RangeError */
  private def toIndex(value: JSValue)(using ctx: JSContext): Int =
    val n = value.toNumber
    if n.isNaN || n.isInfinite || n < 0 then ctx.throwRangeError("ToIndex: argument must be a non-negative integer")
    val intVal = n.toLong
    if intVal < 0 then ctx.throwRangeError("ToIndex: argument must be a non-negative integer")
  ...
  def initialize(ctx: JSContext): Unit =
    ...
    bigIntPrototype.defineProperty("toString", JSValue.Native(bigIntToString), enumerable = false)
    bigIntPrototype.defineProperty("valueOf", JSValue.Native(bigIntValueOf), enumerable = false)
      ...
      callImpl = (args, ctx) =>
        ...
        if args.isEmpty then JSValue.BigInt(java.math.BigInteger.ZERO)
          ...
          try JSValue.BigInt(toBigInt(args(0)))
            ...
            case _: Exception => ctx.throwTypeError(s"Cannot convert ${args(0)} to a BigInt"),
    ...
    bigIntPrototype.defineProperty("constructor", JSValue.Native(bigIntConstructor), enumerable = false)
    bigIntConstructor.funcObj.defineProperty("asIntN", JSValue.Native(bigIntAsIntN), enumerable = false)
    bigIntConstructor.funcObj.defineProperty("asUintN", JSValue.Native(bigIntAsUintN), enumerable = false)
    ctx.global.defineProperty("BigInt", JSValue.Native(bigIntConstructor), enumerable = false)
    ...
    symToStringTag match
      case sym: JSValue.Symbol =>
        bigIntPrototype.initSymbolProperty(sym.value, JSValue.fromString("BigInt"), enumerable = false, writable = false, configurable = true)
      case _ => ()
