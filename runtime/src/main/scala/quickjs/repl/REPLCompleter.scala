...
class REPLCompleter extends Completer:
  ...
  private val keywords = Seq(
    ...
    "typeof", "instanceof", "in", "delete", "new", "_"
  )
  ...
  private val arrayMethods = Seq(
    ...
    "map", "filter", "reduce", "forEach", "find", "indexOf", "includes",
    "join", "sort", "reverse", "concat", "flat", "flatMap"
  )

  private val stringMethods = Seq(
    "length", "charAt", "charCodeAt", "concat", "includes", "endsWith",
    "indexOf", "lastIndexOf", "match", "replace", "search", "slice",
    "split", "startsWith", "substring", "toLowerCase", "toUpperCase",
    "trim", "trimLeft", "trimRight", "padStart", "padEnd"
  )
  ...
  private val consoleMethods = Seq(
    "log", "info", "warn", "error", "debug", "trace", "table", "count",
    "countReset", "group", "groupCollapsed", "groupEnd", "time", "timeLog",
    "timeEnd", "assert", "clear", "dir", "dirxml", "exception", "profile"
  )
    ...
    if word.startsWith(".") then
      ...
      if parts.length == 2 then
        ...
        for prop <- matching do
          candidates.add(new Candidate(s"$objName.$prop", prop, null, null, null, null, true))
    else
  ...
  /** Get properties for a known object.
    *
    * @param objName Object name (e.g., "Math", "Array", "console")
