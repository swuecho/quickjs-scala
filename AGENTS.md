...
**Current Status**: Phase 3 - Substantial language support with most ES2024 features. 1,320 tests passing, 0 failures. 15 test262 smoke test suites. Full test262 sweep (aggregated from per-directory chunks, Sep 18 2026, after the module-instantiation round): 37,423/49,502 passing (92.9% of executed tests; 9,202 skipped by feature config; 167 failures, 2,670 errors, 40 timeouts). The previous sweep was 37,415/92.8% with 2,885 non-passing; this round gained 8 passes with zero regressions. 5 QuickJS C test files all passing. The Runner can execute ordinary scripts (including `.mjs` modules) — see "Scripting support" below.
