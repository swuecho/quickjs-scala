...
**Current Status**: Phase 3 - Substantial language support with most ES2024 features. 789 tests passing, 0 failures. 15 test262 smoke test suites. Full test262 sweep (aggregated from per-directory chunks): 33,499/52,896 passing (86.1% of executed tests, 13,994 skipped by feature config). 5 QuickJS C test files all passing.
...
**Known sweep memory pressure**: with the faster interpreter, heavy tests (arrays with millions of indices, Date/DST stress) allocate much faster. Full-suite runs in one JVM can exhaust the 4GB sbt heap and GC-thrash, because timeout-abandoned work and 8 concurrent workers hold large live arrays. Run the sweep in per-directory chunks instead (each completes), e.g. `built-ins`, `language`, `staging`, `harness`. Engine follow-ups: `JSArray.writeElement` calls `sizeHint(index+1)` and can materialize up to `MaxDenseIndex` (1M) entries on a single large-index write; native builtin loops should honor thread interruption so timed-out workers stop allocating.
