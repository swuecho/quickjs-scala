...
**Current Status**: Phase 3 - Substantial language support with most ES2024 features. 1,325 tests passing, 0 failures. 15 test262 smoke test suites. Full test262 sweep (aggregated from per-directory chunks, Sep 18 2026, after the String protocol round): 37,653/49,502 passing (93.4% of executed tests; 9,202 skipped by feature config; 167 failures, 2,440 errors, 40 timeouts). The previous sweep was 37,593/93.3% with 2,647 non-passing; this round gained 60 passes with zero regressions. 5 QuickJS C test files all passing. The Runner can execute ordinary scripts (including `.mjs` modules) — see "Scripting support" below.
...
- Fixed 39 `built-ins/String/prototype` tests (the `cstm-*-primitive` protocol family, `searchValue-replacer-*`, split ordering/empty-string cases) plus 19 `staging/sm` tests. 62 `String/prototype` errors remain, spread across `match` (13, mostly `index` on fallback matches), `replaceAll` (10, substitution edge cases and `class extends RegExp`), `search` (5) and `replace` (4).
- Regression coverage: 1 new `ConformanceRegressionTest` test (primitive non-boxing, split receiver/order, replaceAll receiver). Full suite now 1,325 tests, 0 failures.
