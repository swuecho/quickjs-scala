...
**Current Status**: Phase 3 - Substantial language support with most ES2024 features. 1,182 tests passing, 0 failures. 15 test262 smoke test suites. Full test262 sweep (aggregated from per-directory chunks, Sep 2026 final rerun): 34,530/49,765 passing (88.7% of executed tests; 10,818 skipped by feature config; 395 failures, 3,972 errors, 50 timeouts). 5 QuickJS C test files all passing. The Runner can execute ordinary scripts (including `.mjs` modules) — see "Scripting support" below.
...
- Still missing for Node-style scripts: `Intl` (and locale-correct `toLocaleString`), `fetch`/`http`, and full structured-clone coverage (Map/Set/TypedArrays/ArrayBuffer).
...
- `NodeCompatTest` (21 tests) covers resolution, cycles, `fs`, Buffer, path, process/assert/events/util/url, ESM interop, package `type`, `process.exit`, crypto/child_process/querystring, and includes a differential test against the real `node` binary when it is on `PATH` (skipped otherwise).
...
- Known differences/limits: `stream`, `readline`, `tty`, `worker_threads`, `http.createServer` and `Intl` are not implemented; `path.win32` aliases POSIX; top-level `await` in modules still only unwraps already-settled promises (async functions get full suspension); `fetch` buffers the whole body (no streaming `response.body`).
...
- A new `AwaitAsync` opcode (async functions only, code 94) always suspends; the top-level `Await` opcode keeps the historical synchronous unwrap for top-level await in modules.
...
**Current Test Count**: 1,182 tests, 0 failures, 0 errors

### Test Distribution
- **stdlib**: 888 tests — language features, built-in objects, JSON, arrays, TypedArrays, Node compatibility (`NodeCompatTest`), etc.
- **runtime**: 203 tests — interpreter correctness, closures, try/catch, classes, etc.
- **compiler**: 13 tests
- **parser**: 78 tests (lexer + parser + strict mode)
- **core**: 16 tests
- **REPL**: 23 tests
