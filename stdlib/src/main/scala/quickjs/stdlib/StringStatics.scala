...
/** JavaScript String object with methods.
  *
  * Provides str.length, str.trim(), str.split(), etc.
...
object StringStatics:
  ...
  def initialize()(using ctx: JSContext): Unit =
    ...
    val trimFunc = NativeFunction("trim", (args, context) =>
    ...
    val toLowerCaseFunc = NativeFunction("toLowerCase", (args, context) =>
      if args.isEmpty then
        JSValue.fromString("")
      else
        args(0) match
    ...
    val toUpperCaseFunc = NativeFunction("toUpperCase", (args, context) =>
      if args.isEmpty then
        JSValue.fromString("")
      else
        args(0) match
    ...
    val indexOfFunc = NativeFunction("indexOf", (args, context) =>
    ...
    val includesFunc = NativeFunction("includes", (args, context) =>
      if args.length < 2 then
        JSValue.Bool(false)
      else
        args(0) match
    ...
    val startsWithFunc = NativeFunction("startsWith", (args, context) =>
      if args.length < 2 then
        JSValue.Bool(false)
      else
        args(0) match
    ...
    val endsWithFunc = NativeFunction("endsWith", (args, context) =>
      if args.length < 2 then
        JSValue.Bool(false)
      else
        args(0) match
    ...
    val splitFunc = NativeFunction("split", (args, context) =>
      if args.isEmpty then
        JSValue.JSArrayVal(quickjs.objmodel.JSArray.empty())
      else
        args(0) match
          case JSValue.JSStr(s) =>
            val sep = if args.length >= 2 then
              args(1) match
    ...
    val substringFunc = NativeFunction("substring", (args, context) =>
      if args.isEmpty then
        JSValue.fromString("")
      else
        args(0) match
    ...
    val charAtFunc = NativeFunction("charAt", (args, context) =>
      if args.isEmpty then
        JSValue.fromString("")
      else
        args(0) match
    ...
    val charCodeAtFunc = NativeFunction("charCodeAt", (args, context) =>
      if args.isEmpty then
        JSValue.fromInt(Double.NaN.toInt)  // NaN
      else
        args(0) match
          case JSValue.JSStr(s) =>
            val index = if args.length >= 2 then args(1).toNumber.toInt else 0
    ...
    val replaceFunc = NativeFunction("replace", (args, context) =>
